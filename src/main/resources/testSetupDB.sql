...
CREATE VIEW `PersonDTO` AS
SELECT *
FROM Person
         INNER JOIN Address USING (address_ID);
CREATE VIEW `AccountDTO` AS
SELECT *
FROM `Account`
         INNER JOIN PersonDTO USING (person_ID);
CREATE VIEW `CarportDTO` AS
SELECT *
FROM Carport
         LEFT JOIN Shack USING (shack_ID);
CREATE VIEW `ProductDTO` AS
SELECT *
FROM Product
         INNER JOIN Product_type USING (product_type_ID);
CREATE VIEW `ProductVariantDTO` AS
SELECT *
FROM Product_variant
         INNER JOIN ProductDTO USING (product_ID)
         INNER JOIN Size USING (size_ID);
CREATE VIEW `lineitemdto` AS
SELECT *
FROM Bill_of_material_line_item
         INNER JOIN productvariantdto USING (product_variant_ID);
